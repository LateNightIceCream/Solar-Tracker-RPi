...
void port_init () {
    ...
    STEPPER_ELV_LIMIT_DDR &=~STEPPER_ELV_LIMIT_BIT;
    STEPPER_ELV_LIMIT_CR1 &=~STEPPER_ELV_LIMIT_BIT;
    STEPPER_ELV_LIMIT_CR2 |= STEPPER_ELV_LIMIT_BIT;
...
static int16_t current_pos_azm = ZERO_POSITION_AZM;
static int16_t current_pos_elv = ZERO_POSITION_ELV;
...
port_c_isr(void) __interrupt(IRQ_EXTI2) {
    ...
    if (ELV_LIMIT_PUSHED) {
        current_pos_elv  = ZERO_POSITION_ELV;
        target_steps = 0;
        stepping_done = 1; // ?
        TIM1_CR1 &= ~CEN;
    }
